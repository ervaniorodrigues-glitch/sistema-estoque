...
# Configuração do SQLite (mais simples, sem necessidade de servidor)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///estoque_web.db'
...
# Desabilitar cache de templates e static files
...
class Cliente(db.Model):
    ...
    nome = db.Column(db.String(200), nullable=False)
    email = db.Column(db.String(100))
    cpfcnpj = db.Column(db.String(18), unique=True)
    telefone = db.Column(db.String(20))
...
def api_fornecedores_listar():
    busca = request.args.get('busca', '')
    ativo = request.args.get('ativo', 'todos')
    filtro_oficina = request.args.get('filtro_oficina', 'false')
    
    query = Fornecedor.query
    
    if busca:
        # Se filtro_oficina está ativo, buscar APENAS pela oficina
        if filtro_oficina == 'true':
            query = query.filter(Fornecedor.oficina.ilike(f'%{busca}%'))
        else:
...
def api_fornecedores_atualizar(codigo):
    ...
    fornecedor.oficina = data.get('oficina')
...
class Movimentacao(db.Model):
    ...
    tipo = db.Column(db.String(20), nullable=False)  # ENTRADA, SAIDA, EMPRESTIMO, DEVOLUCAO
...
def api_movimentacoes_produto(produto_codigo):
    movimentacoes = Movimentacao.query.filter_by(produto_codigo=produto_codigo).order_by(Movimentacao.data_movimentacao.desc()).all()
...
def api_movimentacoes_listar():
    ...
    # Buscar informações do produto para cada movimentação
    resultado = []
    for m in movimentacoes:
        ...
        # Para ENTRADA: mostrar o fornecedor
        # Para SAIDA: mostrar o solicitante (funcionário, cliente, fornecedor)
        operacao_pessoa = ''
        if m.tipo == 'ENTRADA':
            # Para entrada, buscar o fornecedor do produto
            ...
            # Para saída, usar o solicitante cadastrado
...
def api_entrada_multipla():
    ...
    try:
        # Buscar o nome do fornecedor selecionado
...
def api_emprestimos_listar():
    ...
    if descricao:
        emprestimos = [e for e in emprestimos if descricao.lower() in (e.descricao_item or '').lower()]
    ...
    if funcionario:
        emprestimos = [e for e in emprestimos if funcionario.lower() in (e.funcionario or '').lower()]
...
def exportar_emprestimos_excel():
    ...
    for emp in emprestimos:
        ...
        if emp.status == 'EMPRESTADO':
            if emp.data_devolucao_prevista:
                try:
                    ...
                    if isinstance(emp.data_devolucao_prevista, str):
                        data_prevista_str = emp.data_devolucao_prevista.split(' ')[0]
                        data_prevista = datetime.strptime(data_prevista_str, '%d/%m/%Y').date()
                    else:
                        data_prevista = emp.data_devolucao_prevista.date() if hasattr(emp.data_devolucao_prevista, 'date') else emp.data_devolucao_prevista
                ...
                # Calcular há quantos dias está emprestado (desde a data de lançamento)
                try:
                    if isinstance(emp.data, str):
                        data_emp = datetime.strptime(emp.data.split(' ')[0], '%d/%m/%Y').date()
                    else:
                        data_emp = emp.data.date() if hasattr(emp.data, 'date') else emp.data
                    
                    diff = (hoje - data_emp).days
                    if diff == 0:
                        dias = 'Emprestado hoje'
                    elif diff == 1:
                        dias = 'Há 1 dia'
                    else:
                        dias = f'Há {diff} dias'
        ...
        elif emp.status == 'DEVOLVIDO' and emp.data and emp.data_devolucao:
            try:
                # Calcular quantos dias ficou emprestado
                if isinstance(emp.data, str):
                    data_emp = datetime.strptime(emp.data.split(' ')[0], '%d/%m/%Y').date()
                else:
                    data_emp = emp.data.date() if hasattr(emp.data, 'date') else emp.data
                
                if isinstance(emp.data_devolucao, str):
                    data_dev_obj = datetime.strptime(emp.data_devolucao.split(' ')[0], '%d/%m/%Y').date()
                else:
                    data_dev_obj = emp.data_devolucao.date() if hasattr(emp.data_devolucao, 'date') else emp.data_devolucao
                
                diff = (data_dev_obj - data_emp).days
                if diff == 0:
                    dias = 'Mesmo dia'
                elif diff == 1:
                    dias = '1 dia'
...
def get_configuracoes_relatorios():
    try:
        # Forçar refresh dos metadados da tabela
        ...
        if not config:
            ...
            config = ConfiguracaoSistema(
                nome_empresa='ENKYO GUARULHOS',
                rodape='DEUS É BOM O TEMPO TODO'
            )
...
def importar_produtos_excel():
    ...
    try:
        ...
        # Validar colunas esperadas (flexível - aceita variações de nome)
        # IMPORTANTE: Aceita o formato da exportação do próprio sistema
        colunas_esperadas = {
            'codigo': ['Código', 'Codigo', 'CÓDIGO', 'CODIGO', 'Cód', 'COD', 'ID'],
            'descricao': ['Descrição', 'Descricao', 'DESCRIÇÃO', 'DESCRICAO', 'Produto', 'PRODUTO', 'Item', 'ITEM'],
            'codigo_barras': ['Cód.Barras', 'CÓD.BARRAS', 'Código de Barras', 'Codigo de Barras', 'Cod.Barras', 'COD.BARRAS', 'CÓDIGO DE BARRAS', 'CODIGO DE BARRAS', 'Cod Barras', 'COD BARRAS'],
            'unidade': ['Unidade', 'UNIDADE', 'Un', 'UN'],
            'marca': ['Marca', 'MARCA'],
            'categoria': ['Categoria', 'CATEGORIA'],
            'fornecedor': ['Fornecedor', 'FORNECEDOR'],
            'preco_compra': ['R$ Compra', 'R$ COMPRA', 'Preço Compra', 'Preco Compra', 'PREÇO COMPRA', 'PRECO COMPRA', 'Compra', 'COMPRA'],
            'preco_venda': ['R$ Venda', 'R$ VENDA', 'Preço Venda', 'Preco Venda', 'PREÇO VENDA', 'PRECO VENDA', 'Venda', 'VENDA'],
            'estoque_minimo': ['Est.Mín', 'EST.MÍN', 'Estoque Mínimo', 'Estoque Minimo', 'Est.Min', 'EST.MIN', 'ESTOQUE MÍNIMO', 'ESTOQUE MINIMO', 'Est Mín', 'EST MIN'],
            'estoque_maximo': ['Est.Máx', 'EST.MÁX', 'Estoque Máximo', 'Estoque Maximo', 'Est.Max', 'EST.MAX', 'ESTOQUE MÁXIMO', 'ESTOQUE MAXIMO', 'Est Máx', 'EST MAX'],
            'status': ['Status', 'STATUS', 'Ativo', 'ATIVO'],
            'observacao': ['Observação', 'Observacao', 'OBSERVAÇÃO', 'OBSERVACAO', 'Obs', 'OBS']
        }
        
        # Mapear colunas do Excel para campos do banco (busca flexível)
        mapeamento = {}
        for campo, variacoes in colunas_esperadas.items():
            for variacao in variacoes:
                # Buscar com e sem espaços/acentos
                coluna_encontrada = None
                for col in df.columns:
                    col_str = str(col).strip()
                    variacao_str = str(variacao).strip()
                    if col_str.upper() == variacao_str.upper():
        ...
        # Verificar se tem pelo menos a descrição
        if 'descricao' not in mapeamento:
            # Se não encontrou descrição, tentar importar mesmo assim usando a primeira coluna de texto
            # Procurar a primeira coluna que parece ter descrições de produtos
            for col in df.columns:
                # Verificar se a coluna tem valores de texto (não numéricos)
                amostra = df[col].dropna().head(5)
                if len(amostra) > 0:
                    # Se a maioria dos valores são strings longas, provavelmente é descrição
                    textos = [str(x) for x in amostra if not str(x).isdigit()]
                    if len(textos) >= 3:  # Pelo menos 3 valores de texto
                        mapeamento['descricao'] = col
                        break
        
        # Se ainda não encontrou descrição, retornar erro
        if 'descricao' not in mapeamento:
            colunas_disponiveis = ', '.join([str(c) for c in df.columns.tolist()])
            return jsonify({
                'success': False, 
                'message': f'Não foi possível identificar a coluna de descrição dos produtos.\n\nColunas encontradas: {colunas_disponiveis}'
            })
        ...
        def pegar_valor(row, campo, tipo='str', padrao=None):
            """Pega valor da linha com tratamento de erros"""
            try:
                ...
                if pd.isna(valor) or valor == '' or str(valor).strip().upper() in ['NAN', 'NONE', '']:
                    return padrao
        ...
        for index, row in df.iterrows():
            try:
                ...
                # Criar produto com valores seguros
                produto = Produto(
                    descricao=descricao,
...
def exportar_funcionarios_excel():
    ...
    # Preparar dados
    headers = ['ID', 'Nome', 'Cargo', 'CPF', 'Telefone', 'Email', 'Endereço', 'Cidade', 'UF', 'Status']
    dados = []
...
def exportar_fornecedores_excel():
    ...
    if busca:
        query = query.filter(Fornecedor.nome.like(f'%{busca}%'))
    ...
    # Preparar dados
    headers = ['Código', 'Nome/Razão Social', 'CNPJ', 'Telefone', 'Email', 'Oficina', 'Contato', 'Endereço', 'Cidade', 'UF', 'CEP', 'Status']
    dados = []
...
def exportar_clientes_excel():
    ...
    if busca:
        query = query.filter(Cliente.nome.like(f'%{busca}%'))
    ...
    # Preparar dados
    headers = ['ID', 'Nome', 'CPF/CNPJ', 'Telefone', 'Email', 'Endereço', 'Cidade', 'UF', 'CEP', 'Status']
    dados = []
...
def add_no_cache_headers(response):
    """Desabilitar cache em todas as respostas para evitar problemas de dados antigos"""
